...
function openCart() {
  ...
  let rSubtotal = document.createElement("p");
  rSubtotal.innerText = `Subtotal: $${subtotalNum}`;
  receiptPage.appendChild(rSubtotal);
  let taxEl = document.createElement("p");
  let taxNum = Math.round(subtotalNum * 0.06 * 100) / 100;
  taxEl.innerText = `Tax (6% local) $${taxNum}`;
  let rtaxEl = document.createElement("p");
  rtaxEl.innerText = `Tax (6% local) $${taxNum}`;
  cartPage.appendChild(taxEl);
  receiptPage.appendChild(rtaxEl);
  let total = document.createElement("p");
  let totalNum = subtotalNum + taxNum;
  let rTotal = document.createElement("p");
  total.innerText = `Total: $${totalNum}`;
  rTotal.innerText = `Total: $${totalNum}`;
  cartPage.appendChild(total);
  receiptPage.appendChild(rTotal);
